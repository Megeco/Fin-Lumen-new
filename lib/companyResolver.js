...
async function loadDynamicRegistry() {
  ...
  // Personal deployments use bundled charts plus any process-local research
  // overrides. Watchlists are stored in each visitor's browser.
  dynamicCache = mergeRuntimeMemoryEntries({}, getMemoryNatalEntries());
...
async function resolveCompany(ticker, stockOverride = null, options = {}) {
  ...
  if (registrySnapshot) {
    return {
      found: true,
      symbol,
      registrySource: "request-natal-registry",
      registryType: "USER",
      locked: false,
  ...
  if (dynamicCompany?.incorporationDate || dynamicCompany?.listingDate || dynamicCompany?.birthDate) {
    return {
      found: true,
      symbol,
      registrySource: "runtime-natal-registry",
      registryType: "USER",
      locked: false,
