  ...
  return <main>
    ...
    <div className="shell" id="top">
      ...
      <section className="workspace">
        ...
        <div className="table-foot"><span>Showing {visible.length} of {watchlist.length} readings</span><span>Swiss Ephemeris · Lahiri · Engine v37.9.14</span><span>Scores describe astrological structure and intensity; they are not financial forecasts.</span></div>
...
function readingMeaning(s:Stock){return s.model?.paths?.tactical?.[0]?.expectedExpression||s.story}

function StockReading({s,deep=false}:{s:Stock;deep?:boolean}){
  return <div className="stock-reading">
    <section className="reading-lead"><span className="eyebrow">Reading · {displayDate(s.skyDate)}</span><TonePill tone={s.tone}>{s.reading}</TonePill><h2>{s.story}</h2><p className="phase-meaning"><b>What this means:</b> {readingMeaning(s)}</p></section>
...
function AddStockModal({stocks,watchlist,onRequestAdd,onClose}:{stocks:Stock[];watchlist:string[];onRequestAdd:(value:string)=>Promise<AddResult>;onClose:()=>void}){
  ...
  return <div className="drawer-backdrop modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="add-modal" role="dialog" aria-modal="true" aria-label="Add a stock"><button className="close" onClick={onClose}>×</button><span className="eyebrow">Add to My Stocks · {watchlist.length}/100</span><h2>Enter a company or ticker.</h2><p>Choose a company with a natal chart in this build. Adding it changes only your own table. Unknown companies need a reviewed chart before they can be included.</p><div className="add-search"><input autoFocus value={query} onChange={e=>reset(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")void check()}} placeholder="For example: MARUTI.NS or ABB.NS"/><button onClick={()=>void check()} disabled={!query.trim()||loading||watchlist.length>=100}>{loading?"Checking…":"Find company"}</button></div>{loading&&<p className="checking-note">Calculating the current reading from the bundled natal chart. This can take a few seconds.</p>}{!checked&&<div className="suggestions"><span>{ENGINE_SUPPORTED_SYMBOLS.length} engine-supported symbols · try:</span>{["MARUTI","ABB","TITAN","COCHINSHIP"].map(x=><button key={x} onClick={()=>reset(x)}>{x}</button>)}</div>}
      ...
      {result?.status==="added"&&!known&&<div className="availability available"><span>✓</span><div><b>Added to My Stocks</b><p>{result.message}</p><small>Its approved reading is now available without recalculating the rest of the table.</small></div><button onClick={onClose}>View table</button></div>}
      {result&&["preparing-evidence","candidate-ready","owner-review"].includes(result.status)&&<div className={`availability ${result.status==="owner-review"?"reviewing":"preparing"}`}><span>{result.status==="owner-review"?"◇":result.status==="candidate-ready"?"✓":"◷"}</span><div><b>{result.status==="owner-review"?"Owner verification required":result.status==="candidate-ready"?"Official listing candidate prepared":"Company request recorded"}</b><p>{result.message}</p><small>No reading will appear until official evidence is checked and a natal basis is approved.</small></div><button onClick={onClose}>Done</button></div>}
      {result&&!["added","preparing-evidence","candidate-ready","owner-review"].includes(result.status)&&<div className={`availability ${result.status==="engine-unreachable"||result.status==="error"?"preparing":"unavailable"}`}><span>{result.status==="engine-unreachable"?"↻":"○"}</span><div><b>{result.status==="engine-unreachable"?"Astro engine temporarily unavailable":result.status==="identity-needed"?"Exact ticker needed":"Reading unavailable"}</b><p>{result.message}</p><small>{result.status==="identity-needed"?"Use an exchange ticker so the legal company is not guessed.":"Nothing has been added to the approved universe."}</small></div><button onClick={()=>{setChecked(false);setResult(null)}}>{result.status==="identity-needed"?"Enter ticker":"Try again"}</button></div>}
